...
class AuthenticService {
    ...
    async deployUpgradeable(admin: any, param: any) {
        ...
        const contract = await ethers.getContractFactory("Ordinals");
        console.log("Ordinals.deploying ...")
        const proxy = await upgrades.deployProxy(contract, [admin, param], {
            initializer: 'initialize(address, address)',
        });
        await proxy.deployed();
        console.log("Ordinals deployed at proxy:", proxy.address);
        return proxy.address;
    }

    getContract(contractAddress: any, contractName: any = "./artifacts/contracts/services/Ordinals.sol/Ordinals.json") {
    ...
    async upgradeContract(proxyAddress: any) {
        const contractUpdated = await ethers.getContractFactory("Ordinals");
        console.log('Upgrading Ordinals... by proxy ' + proxyAddress);
        const tx = await upgrades.upgradeProxy(proxyAddress, contractUpdated);
        console.log('Ordinals upgraded on tx address ' + tx.address);
        return tx;
    }
    ...
    async setCaller(contractAddress: any, caller: any, approve: any, gas: any) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        let fun = temp?.nftContract.methods.setCaller(caller, approve);
        //the transaction
        const tx = {
