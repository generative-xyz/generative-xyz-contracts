...
async function main() {
  ...
  const attributeCounts: {
    [key: string]: { [key: string]: { counter: number; percent: number } };
  } = {};
  let totalTokens = 0;
  ...
  for (let i = 1; i <= num; i++) {
    try {
      ...
      // attr = JSON.parse(attr).filter((item: any) => item.trait_type !== "DNA");
      ...
      if (attrsChecked.includes(attrStr)) {
        ...
        console.log(
          `Duplicate of ID ${duplicateId}:`,
          JSON.parse(attrsChecked[duplicateIndex])
        );
      }
      ...
      attributes.forEach((attribute: any) => {
        ...
        attributeCounts[trait][value].percent = Number(
          ((attributeCounts[trait][value].counter / totalTokens) * 100).toFixed(
            2
          )
        );
      });
  ...
  const rarityData: {
    [key: string]: { [key: string]: { percent: number; counter: number } };
  } = {};
