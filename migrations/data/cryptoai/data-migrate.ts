...
const getColorIndex = (color: number[]): number => {
  ...
  let index = colorPalette.findIndex(
    ...
    try {
      if (fs.existsSync(PATH_OUTPUT_ERRORS)) {
        existingErrors = JSON.parse(
          fs.readFileSync(PATH_OUTPUT_ERRORS, "utf-8")
        );
      }
      ...
      fs.writeFileSync(PATH_OUTPUT_ERRORS, JSON.stringify(errors, null, 2));
    }
  }
...
const convertAssetsToJson = (
  ...
  try {
    ...
    svgFiles.forEach((filePath: string) => {
      ...
      const [nameSubFolderTitle, subFolderTrait] = subFolder.split("_");

      const subFolderTitle = nameSubFolderTitle.split(".")[1];
