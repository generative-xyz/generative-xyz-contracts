...
async function main() {
  ...
  for(let i = 0; i < collections.length; i++) {
    ...
    const elements_remove_empty = elements.filter((element: any) => element !== '');
    ...
    elements.forEach((element: any, index: number) => {
      ...
      const pr_key = index === 0 ? 'dna' : index === 1 ? 'body' : index === 2 ? 'head' : index === 3 ? 'eyes' : index === 4 ? 'mouth' : 'unknown';
      if (!data[pr_key]) { 
        data[pr_key] = {};
      }
