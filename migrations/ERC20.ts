...
class ERC20 {
    ...
    getContract(contractAddress: any, contractName: any = "./artifacts/@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol/ERC20Upgradeable.json") {
    ...
    async increaseAllowance(contractAddress: any, spender: any, amount: any, gas: any) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.increaseAllowance(spender, amount)
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
    ...
    async decreaseAllowance(contractAddress: any, spender: any, amount: any, gas: any) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce

        const fun = temp?.nftContract.methods.decreaseAllowance(spender, amount)
        //the transaction
        const tx = {
            from: this.senderPublicKey,
            to: contractAddress,
