...
class Mempool {
    ...
    async upgradeContract(proxyAddress: any) {
        const contractUpdated = await ethers.getContractFactory("Mempool");
        console.log('Upgrading Mempool... by proxy ' + proxyAddress);
        const tx = await upgrades.upgradeProxy(proxyAddress, contractUpdated);
        console.log('Mempool upgraded on tx address ' + tx.address);
        return tx;
    }
    ...
    getContract(contractAddress: any) {
        ...
        // load contract
        let contract = require(path.resolve("./artifacts/contracts/nfts/Mempool.sol/Mempool.json"));
    ...
    async mint(contractAddress: any, to: any, gas: any, nonce: any) {
        let temp = this.getContract(contractAddress);
        // const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce
