...
function generateRandomData(data: any, indexGen: number, indexDNA: number, rageDNA: number): { name: any, index: any } {
  ...
  try {
    ...
    const indexNameMouth = traitsElement(data.elements[ELEMENT.MOUTH].traits, mouthSeed, rageDNA);
    const indexNameHead = traitsElement(data.elements[ELEMENT.HEAD].traits, headSeed, rageDNA);
    const indexNameEyes = traitsElement(data.elements[ELEMENT.EYES].traits, eyesSeed, rageDNA);
    const indexNameEarring = traitsElement(data.elements[ELEMENT.EARRING].traits, earringSeed, rageDNA);
  

    const randomDataIndex = [
        indexDNA,
        [indexNameDNAType, indexNameEarring, indexNameHead, indexNameEyes, indexNameMouth]
    ];

    const randomData = [
        dnaKey,
        [data.DNA[dnaKey].names[indexNameDNAType], data.elements[ELEMENT.EARRING].names[indexNameEarring], data.elements[ELEMENT.HEAD].names[indexNameHead], data.elements[ELEMENT.EYES].names[indexNameEyes], data.elements[ELEMENT.MOUTH].names[indexNameMouth]]
    ];
...
async function main() {
  ...
  try {
    ...
    while (indexArt <= num) {
      ...
      if(dataCompress.elements[ELEMENT.EARRING].traits[data.index[1][1]] < rageTriggerDrop) {
        dataCompress.elements[ELEMENT.EARRING].traits[data.index[1][1]] *= dropTraitValue;
      }

      if(dataCompress.elements[ELEMENT.HEAD].traits[data.index[1][2]] < rageTriggerDrop) {
        dataCompress.elements[ELEMENT.HEAD].traits[data.index[1][2]] *= dropTraitValue;
      }

      if(dataCompress.elements[ELEMENT.EYES].traits[data.index[1][3]] < rageTriggerDrop) {
        dataCompress.elements[ELEMENT.EYES].traits[data.index[1][3]] *= dropTraitValue;
      }

      if(dataCompress.elements[ELEMENT.MOUTH].traits[data.index[1][4]] < rageTriggerDrop) {
        dataCompress.elements[ELEMENT.MOUTH].traits[data.index[1][4]] *= dropTraitValue;
      }
