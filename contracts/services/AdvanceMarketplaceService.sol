...
contract AdvanceMarketplaceService is SimpleMarketplaceService {
    ...
    function setMultiBuyOffering(Marketplace.MultiBuyOffering memory offer, address erc721, uint256 tokenId) external nonReentrant payable {
        ...
        if (offer.erc20Token == Errors.ZERO_ADDR) {
            ...
            IERC20Upgradeable erc20 = IERC20Upgradeable(offer.erc20Token);
            require(erc20.allowance(msg.sender, address(this)) >= _multiBuyOffering[erc721][tokenId].budget, Errors.ERC20_NOT_APPROVED);
            require(erc20.transferFrom(msg.sender, address(this), _multiBuyOffering[erc721][tokenId].budget), Errors.TRANSFER_FAIL);
        }
    }

    function cancelMultiBuyOffering(address erc721, uint256 tokenId) external nonReentrant {
        require(_multiBuyOffering[erc721][tokenId].buyer == msg.sender);
        if (_multiBuyOffering[erc721][tokenId].budget > 0) {
            if (_multiBuyOffering[erc721][tokenId].erc20Token == Errors.ZERO_ADDR) {
                (bool success,) = _multiBuyOffering[erc721][tokenId].buyer.call{value : _multiBuyOffering[erc721][tokenId].budget}("");
                require(success);
            } else {
                IERC20Upgradeable erc20 = IERC20Upgradeable(_multiBuyOffering[erc721][tokenId].erc20Token);
                require(erc20.transferFrom(address(this), _multiBuyOffering[erc721][tokenId].buyer, _multiBuyOffering[erc721][tokenId].budget), Errors.TRANSFER_FAIL);
            }
        }
    }
    ...
    function listToken(address hostContractErc721, uint tokenId, address erc20Token, uint price) external override nonReentrant returns (bytes32) {
        ...
        if (_multiBuyOffering[hostContractErc721][tokenId].buyer != Errors.ZERO_ADDR) {
            if (_multiBuyOffering[hostContractErc721][tokenId].erc20Token == _offeringRegistry[id].erc20Token) {
